=======================
DSAT distributed syste
=======================

References
==========

source :  http://github.com/jul/dsat
...
- you can optionnaly add a load handler in the tracker (an example is provided) does the monitoring of the load and the respawning of new process if needed;
- the Status of a pipelined task can be followed either dynamically (cf status_for)
  or statically by requesting the State(s) object (to be changed);
...
Doable, your way. I think people should implement their logic, not mine.
...
state.get_connection set the connections of your step magically 
...
* change the state tracker from mongo to a fixed size structure with expiring (using dogpile.cache, lru or whatever);
* understand why circus is so unresponsive
* create a makefile to install binary dependancies python packaging cannot.
